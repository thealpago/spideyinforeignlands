...
export const useVehicleInput = () => {
	...
	const getVehicleInput = useCallback((delta, forwardSpeed = 0) => {
		...
		// Throttle input (forward) - combine keyboard and touch
		const throttleInput = clamp(
			(effectiveKeys.has('ArrowUp') || effectiveKeys.has('w') ? 1 : 0) + 
			(input.rightStickY < 0 ? -input.rightStickY : 0) +
			(touchInput.leftStickY < 0 ? -touchInput.leftStickY : 0)
		)

		// Brake input (backward/brake) - combine keyboard and touch
		const brakeInput = clamp(
			(effectiveKeys.has('ArrowDown') || effectiveKeys.has('s') ? 1 : 0) + 
			(input.rightStickY > 0 ? input.rightStickY : 0) +
			(touchInput.leftStickY > 0 ? touchInput.leftStickY : 0)
		)
		...
		// Combine smoothed keyboard steering with analog stick and touch input
		const steerInput = clamp(
			smoothedKeyboardSteering.current + 
			-input.leftStickX + 
			-touchInput.leftStickX
		)
